...
install_remote_runtime() {
  ...
  gcloud_ssh "chmod +x '$remote_script' && sudo '$remote_script' && rm -f '$remote_script'"
}
...
deploy_compose_stack() {
  ...
  gcloud_ssh "chmod +x '$remote_script' && sudo '$remote_script' '$REMOTE_APP_DIR' '$COMPOSE_PROJECT_NAME' && rm -f '$remote_script'"
}
